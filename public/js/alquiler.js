...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const venceEl = document.getElementById('factura-vence');
    if (venceEl) venceEl.value = expires.toISOString().split('T')[0];

    // Número de factura automático
    const noEl = document.getElementById('factura-no');
    if (noEl) noEl.value = Math.floor(Math.random() * 9000) + 1000;
...
function recalcularTotales() {
    ...
    const valorLetrasEl = document.getElementById('valor-letras');

    if (totalItemsEl) totalItemsEl.textContent = itemsCount;
    if (totalBrutoEl) totalBrutoEl.textContent = formatCOP(totalBruto);
    if (totalPagarEl) totalPagarEl.textContent = formatCOP(totalBruto);
    if (valorLetrasEl) valorLetrasEl.textContent = totalBruto > 0 ? numeroALetras(totalBruto) : '—';
}
